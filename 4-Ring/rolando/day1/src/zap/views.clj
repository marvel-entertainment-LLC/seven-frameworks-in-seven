  ...
  (base-page
   ...
   [:ol
    (for [p (models/all-projects)] ;;(4)
      [:li 
        [:a {:href (str "/project/" (:id p) "/issues")} (:name p) " (" (models/issue-count (:id p)) " issues)"]
        " | " [:a {:href (str "/projects/" (:id p))} :edit]])]))
;
...
(defn issue [id]
  (let [iss (models/issue-by-id id)]
    (base-page
     ...
     [:h1 "#" id ": " (:title iss) " "
      [:a {:href (str "/issues/" (:id iss)) :class "btn btn-primary"} "edit"]]
...
(defn edit-project [id]
  (let [proj (models/project-by-id id)]
    (base-page
     ...
     [:h1 "Editing " (:name proj)]
     (form-to
      {:class :form-horizontal}
      [:post (str "/projects/" (:id proj))]
      (text-field :name (:name proj))
      (submit-button {:class "btn btn-primary"} "Update Project")))))

(defn update-project [id params]
  (models/update-project id params) ; update-project method was already in models
  (response/redirect-after-post "/projects"))
...
(defn edit-issue [id]
  (let [iss (models/issue-by-id id)]
    (base-page
     ...
     [:h1 "Editing Issue"]
     (form-to
      [:post (str "/issues/" (:id iss))]
      (text-field {:class "span8"
                   :type :text
                   :placeholder "Title"} :title (:title iss))
      [:br]
      (text-area {:class "span8"
                  :placeholder "Description"
                  :rows 5} :description (:description iss))
      [:br]
      (submit-button {:class "btn btn-primary"} "Update Issue")))))

(defn update-issue [id params]
  (models/update-issue id params) ; update-project method was already in models
  (response/redirect-after-post (str "/issue/" id)))